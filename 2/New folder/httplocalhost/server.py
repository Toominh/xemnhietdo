...
def get_network():
    ...
    # WMI: lấy tên model phần cứng NIC thật (Wi-Fi/LAN card name)
    wmi_adapters = {}  # mac_lower -> { model, manufacturer, net_type, driver }
    if WMI:
        try:
            for nic in WMI.Win32_NetworkAdapter():
                if not nic.MACAddress:
                    continue
                mac_key = nic.MACAddress.lower().replace("-", ":").replace(" ", "")
                net_type = "N/A"
                if nic.AdapterType:
                    t = nic.AdapterType.lower()
                    if "wireless" in t or "802.11" in t or "wi-fi" in t or "wifi" in t:
                        net_type = "Wi-Fi"
                    elif "ethernet" in t or "802.3" in t:
                        net_type = "LAN (Ethernet)"
                    else:
                        net_type = nic.AdapterType
                wmi_adapters[mac_key] = {
                    "model":        (nic.Name or "N/A").strip(),
                    "manufacturer": (nic.Manufacturer or "N/A").strip(),
                    "net_type":     net_type,
                    "adapter_type": nic.AdapterType or "N/A",
                    "product_name": (nic.ProductName or "N/A").strip(),
                    "pnp_id":       (nic.PNPDeviceID or "N/A").strip(),
                }
        except: pass

        # Win32_NetworkAdapterConfiguration bổ sung DHCP, DNS
        wmi_cfg = {}
        try:
            for cfg in WMI.Win32_NetworkAdapterConfiguration(IPEnabled=True):
                if cfg.MACAddress:
                    k = cfg.MACAddress.lower().replace("-",":").replace(" ","")
                    wmi_cfg[k] = {
                        "dhcp":      "Bật" if cfg.DHCPEnabled else "Tắt",
                        "dhcp_srv":  cfg.DHCPServer or "N/A",
                        "dns":       ", ".join(cfg.DNSServerSearchOrder) if cfg.DNSServerSearchOrder else "N/A",
                        "gateway":   ", ".join(cfg.DefaultIPGateway) if cfg.DefaultIPGateway else "N/A",
                        "subnet":    ", ".join(cfg.IPSubnet) if cfg.IPSubnet else "N/A",
                    }
        except: pass
    else:
        wmi_cfg = {}
    ...
    for name, stat in stats.items():
        ...
        nic_io  = io.get(name)
        mac_key = mac.lower().replace("-",":").replace(" ","") if mac != "N/A" else ""

        hw_info = wmi_adapters.get(mac_key, {})
        cfg_info = wmi_cfg.get(mac_key, {})

        nets.append({
            "name":         name,
            "model":        hw_info.get("model", "N/A"),
            "manufacturer": hw_info.get("manufacturer", "N/A"),
            "product_name": hw_info.get("product_name", "N/A"),
            "net_type":     hw_info.get("net_type", "N/A"),
            "adapter_type": hw_info.get("adapter_type", "N/A"),
            "pnp_id":       hw_info.get("pnp_id", "N/A"),
            "up":           stat.isup,
            "speed":        f"{stat.speed} Mbps" if stat.speed else "N/A",
            "mtu":          stat.mtu,
            "ipv4":         ipv4,
            "ipv6":         (ipv6[:45] + "...") if len(ipv6) > 45 else ipv6,
            "mac":          mac,
            ...
            "dhcp":         cfg_info.get("dhcp", "N/A"),
            "dhcp_srv":     cfg_info.get("dhcp_srv", "N/A"),
            "dns":          cfg_info.get("dns", "N/A"),
            "gateway":      cfg_info.get("gateway", "N/A"),
            "subnet":       cfg_info.get("subnet", "N/A"),
        })
    return nets


def get_sound():
    """Lấy thông tin card âm thanh qua WMI."""
    devices = []
    if WMI:
        try:
            for s in WMI.Win32_SoundDevice():
                devices.append({
                    "name":         (s.Name or "N/A").strip(),
                    "manufacturer": (s.Manufacturer or "N/A").strip(),
                    "status":       (s.Status or "N/A").strip(),
                    "device_id":    (s.DeviceID or "N/A").strip(),
                    "pnp_id":       (s.PNPDeviceID or "N/A").strip(),
                    "driver_name":  (getattr(s, "DriverName", None) or "N/A").strip(),
                })
        except: pass
    return devices
