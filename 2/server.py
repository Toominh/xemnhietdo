...
try:
    import wmi
    WMI = wmi.WMI()
    WMI_TEMP = wmi.WMI(namespace="root\\OpenHardwareMonitor") if os.path.exists(
        r"C:\Program Files\OpenHardwareMonitor\OpenHardwareMonitor.exe") else None
except:
    WMI = None
    WMI_TEMP = None
...
def get_temps():
    ...
    # OpenHardwareMonitor nếu có
    if WMI_TEMP:
        try:
            for sensor in WMI_TEMP.Sensor():
                if sensor.SensorType == "Temperature":
                    cat = sensor.Parent.split("/")[-1] if "/" in sensor.Parent else "System"
                    if cat not in temps: temps[cat] = []
                    temps[cat].append({"label": sensor.Name, "current": sensor.Value, "high": None, "critical": None})
        except: pass
    return temps
