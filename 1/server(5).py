...
# ══════════════════════════════════════════════════════════════════════════════
def collect_all_sensors(node, result=None):
    """Duyệt đệ quy, trả về list mọi sensor có giá trị."""
    if result is None:
        result = []
    val  = node.get("Value","")
    text = node.get("Text","")
    if val and val.strip() not in ("-","") and text:
        # Xác định loại sensor từ đơn vị
        v = val.strip()
        if "°c" in v.lower():
            stype = "Temperature"
        elif "rpm" in v.lower():
            stype = "Fan"
        elif "v" == v.split()[-1].lower() if v.split() else False:
            stype = "Voltage"
        elif "%" in v:
            stype = "Load"
        elif "w" == v.split()[-1].lower() if v.split() else False:
            stype = "Power"
        elif "mhz" in v.lower() or "ghz" in v.lower():
            stype = "Clock"
        elif "gb" in v.lower() or "mb" in v.lower() or "kb" in v.lower():
            stype = "Data"
        else:
            stype = "Other"
        result.append({"name": text, "value": v, "type": stype})
    for child in node.get("Children", []):
        collect_all_sensors(child, result)
    return result

def build_hardware_info():
    """Trả về list hardware đầy đủ với mọi sensor."""
    global _lhm_hw_cache
    ...
    try:
        ...
        for hw in hw_nodes:
            sensors = collect_all_sensors(hw)
            if not sensors:
                continue
            # Nhóm theo loại sensor
            grouped = {}
            for s in sensors:
                t = s["type"]
                if t not in grouped:
                    grouped[t] = []
                grouped[t].append({"name": s["name"], "value": s["value"]})
            result.append({
                "name":    hw.get("Text",""),
                "type":    classify(hw.get("Text","")),
                "sensors": grouped,
        ...
        # Fallback từ cache
        out = []
        for hw in _lhm_hw_cache:
            grouped = {}
            for t in hw["temps"]:
                grouped.setdefault("Temperature",[]).append({"name":t["name"],"value":f"{t['temp']} °C"})
            for l in hw["loads"]:
                grouped.setdefault("Load",[]).append({"name":l["name"],"value":f"{l['load']} %"})
            if grouped:
                out.append({"name":hw["name"],"type":hw["type"],"sensors":grouped})
        return out
