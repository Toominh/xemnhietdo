...
def get_battery_health():
    """
    Sức khoẻ pin qua WMI root\wmi.
    Trả về (health_pct, full_cap_mwh, design_cap_mwh) hoặc (None,None,None).
    """
    try:
        import wmi as _wmi
        ...
        if full_list and design_list:
            full   = full_list[0].FullChargedCapacity    # mWh hiện tại
            design = design_list[0].DesignedCapacity     # mWh gốc
            if design > 0:
                health = round(full / design * 100, 1)
                return health, full, design
    except Exception:
        pass
    return None, None, None

def get_battery_lhm(hw_list):
    """Đọc nhiệt độ pin + công suất (W) từ LHM."""
    temp, watts, charge_pct = None, None, None
    for hw in hw_list:
        n = hw["name"].lower()
        if "batter" not in n and "acpi" not in n:
            continue
        for t in hw["temps"]:
            if temp is None and 0 < t["temp"] < 80:
                temp = t["temp"]
        for load in hw["loads"]:
            ln = load["name"].lower()
            if "charge level" in ln or "remaining" in ln:
                charge_pct = load["load"]
            elif "charge rate" in ln or "watt" in ln or "power" in ln:
                watts = load["load"]
    return temp, watts, charge_pct

def get_battery_info(hw_list):
    """Tổng hợp tất cả thông tin pin."""
    bat = psutil.sensors_battery()
    if bat is None:
        return None   # không có pin (desktop)

    health, full_cap, design_cap = get_battery_health()
    bat_temp, watts, lhm_pct = get_battery_lhm(hw_list)

    status = "charging"   if bat.power_plugged and bat.percent < 100 else              "full"       if bat.power_plugged and bat.percent >= 100 else              "discharging"
...
def collect():
    ...
    try:
        ...
        battery             = get_battery_info(hw_list)
