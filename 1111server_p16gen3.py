...
def get_cpu_temp(hw_list):
    """Nhiệt độ CPU — ưu tiên Package / P-core / CPU."""
    for hw in hw_list:
        if hw["type"] != "cpu":
            continue
        # Meteor Lake: ưu tiên "CPU Package" hoặc "Core Max"
        for priority in ["package", "core max", "tdie", "tctl", "cpu"]:
            for t in hw["temps"]:
                if priority in t["name"].lower():
                    return t["temp"]
        # Fallback: nhiệt độ cao nhất
        if hw["temps"]:
